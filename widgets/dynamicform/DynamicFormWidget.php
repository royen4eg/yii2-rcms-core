...
class DynamicFormWidget extends Widget
{
    ...
    const WIDGET_FUNCTION = 'rcmsDynamicForm';
    ...
    /**
     * @var Model|yii\db\ActiveRecord the model used for the form
     */
    public $model;
    /**
     * @var string form ID
    ...
    protected function initOptions()
    {
        ...
        if(!empty($this->moveBackwardButton)){
            $this->_options['moveBackwardButton'] = $this->moveBackwardButton;
        }
        if(!empty($this->moveForwardButton)){
            $this->_options['moveForwardButton'] = $this->moveForwardButton;
        }
    ...
    protected function hashOptions()
    {
        ...
        $this->_hashVar = self::WIDGET_ALIAS . '_' . hash('crc32', $this->_encodedOptions);
    }
    ...
    {
        if (isset(Yii::$app->params[self::WIDGET_ALIAS][$this->widgetContainer])) {
            return Yii::$app->params[self::WIDGET_ALIAS][$this->widgetContainer];
        }
    ...
    {
        if (!isset(Yii::$app->params[self::WIDGET_ALIAS][$this->widgetContainer])) {
            Yii::$app->params[self::WIDGET_ALIAS][$this->widgetContainer] = $this->_hashVar;
            return true;
        }
        return false;
    }
    ...
    {
        DynamicFormAssets::register($view);
        ...
        $js .= '    jQuery(".' . $this->widgetContainer . '").' . self::WIDGET_FUNCTION . '("addItem", ' . $this->_hashVar . ", e, jQuery(this));\n";
        $js .= "});\n";
        $view->registerJs($js, $view::POS_READY);
        // add a click handler for the remove button
        $js = 'jQuery("#' . $this->formId . '").on("click", "' . $this->deleteButton . '", function(e) {' . "\n";
        $js .= "    e.preventDefault();\n";
        $js .= '    jQuery(".' . $this->widgetContainer . '").' . self::WIDGET_FUNCTION . '("deleteItem", ' . $this->_hashVar . ", e, jQuery(this));\n";
        $js .= "});\n";
        $view->registerJs($js, $view::POS_READY);

        if (!empty($this->moveBackwardButton)) {
            // add a click handler for the move backward button
            $js = 'jQuery("#' . $this->formId . '").on("click", "' . $this->moveBackwardButton . '", function(e) {' . "\n";
            $js .= "    e.preventDefault();\n";
            $js .= '    jQuery(".' . $this->widgetContainer . '").' . self::WIDGET_FUNCTION . '("moveBackward", ' . $this->_hashVar . ", e, jQuery(this));\n";
            $js .= "});\n";
            $view->registerJs($js, $view::POS_READY);
        }

        if (!empty($this->moveForwardButton)) {
            // add a click handler for the move forward button
            $js = 'jQuery("#' . $this->formId . '").on("click", "' . $this->moveForwardButton . '", function(e) {' . "\n";
            $js .= "    e.preventDefault();\n";
            $js .= '    jQuery(".' . $this->widgetContainer . '").' . self::WIDGET_FUNCTION . '("moveForward", ' . $this->_hashVar . ", e, jQuery(this));\n";
            $js .= "});\n";
            $view->registerJs($js, $view::POS_READY);
        }

        $js = 'jQuery("#' . $this->formId . '").' . self::WIDGET_FUNCTION . '(' . $this->_hashVar . ');' . "\n";
